...
```
위 참고 이미지와 동일한 스타일로, 아래 동물을 기분별로 4장 생성해주세요. 하나의 이미지가 아닌 기분별 png 파일로 총 4장을 생성해야 합니다. 파일명은 {animal}_{mood}.png로 통일, {animal}은 하단의 '생성할 동물'에서 명시한 동물의 영어 이름을 소문자로, {mood}는 아래 각 기분별 효과의 이름 (sleeping, relaxed, active, storm)을 사용하세요.
...
- sleeping → 누운 포즈, 완전히 닫힌 눈 (아래로 볼록한 곡선), 파란색 ZZZ
- relaxed → 눈 닫힘 (눈웃음처럼), 파란색 음표 ♪♫
- active  → 밝은 눈, 팔 들어올림, 노란색 별 ✨
- storm   → 눈 활짝 열림 + 강렬하게 응시하는 표정, 주황색 번개 ⚡ + 불꽃 효과
